<template>
  <div>
    <section class="overflow-hidden">
      <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="relative">
          <h1>
            <span
              class="block text-2xl text-center text-indigo-600 font-semibold tracking-wide uppercase"
              >Why we build</span
            >
          </h1>
          <blockquote class="mt-10">
            <div
              class="max-w-5xl mx-auto text-left text-2xl leading-9 text-gray-900"
            >
              <p class="mt-6">
                While we have a plethora of commerial and open-source database
                engines serving billions of users, our tooling around making
                <span class="underline inline font-semibold">reliable</span>,
                <span class="underline inline font-semibold">consistent</span>,
                <span class="underline inline font-semibold"></span
                ><span class="underline inline font-semibold">auditable</span>
                application database schema changes are embrassing. And today
                many teams are still connecting to the production DB and
                applying changes directly. Quick and dirty, until BOMB!
              </p>
              <p class="mt-6"></p>
              <p class="mt2">
                What's the
                <a
                  href="https://about.gitlab.com"
                  target="_blank"
                  class="text-blue-600 font-semibold"
                  style="box-shadow: rgb(255, 255, 255) 0px -0.166667em 0px 0px inset, rgb(186, 230, 253) 0px -0.333333em 0px 0px inset;"
                  >GitLab</a
                >
                for database instead of code?
              </p>
              <p class="mt-2">
                What's the
                <a
                  href="https://figma.com"
                  target="_blank"
                  class="text-blue-600 font-semibold"
                  style="box-shadow: rgb(255, 255, 255) 0px -0.166667em 0px 0px inset, rgb(186, 230, 253) 0px -0.333333em 0px 0px inset;"
                  >Figma</a
                >
                for DBAs instead of designers?
              </p>
              <p class="mt-2">
                What's the
                <a
                  href="https://terraform.io"
                  target="_blank"
                  class="text-blue-600 font-semibold"
                  style="box-shadow: rgb(255, 255, 255) 0px -0.166667em 0px 0px inset, rgb(186, 230, 253) 0px -0.333333em 0px 0px inset;"
                  >Terraform</a
                >
                for Database-as-Code instead of Infrastructure-as-Code?
              </p>

              <p class="mt-6">There was no really good answer.</p>
              <p class="mt-6">
                It's like a Porsche with a good engine, but only paired with the
                handy wheel and brake, can it deliver the safe and pleasant
                driving experience. And that's what
                <span
                  class="text-indigo-600 font-semibold"
                  style="box-shadow: rgb(255, 255, 255) 0px -0.166667em 0px 0px inset, rgb(186, 230, 253) 0px -0.333333em 0px 0px inset;"
                  >Bytebase</span
                >
                tries to achieve, to pair the database engine with the proper
                wheel and brake to improve the development velocity and
                experience.
              </p>
            </div>
          </blockquote>
        </div>
      </div>
    </section>

    <div class="bg-white">
      <div class="mx-auto py-12 px-4 max-w-7xl sm:px-6 lg:px-8 lg:py-24">
        <div class="space-y-12 lg:grid lg:grid-cols-3 lg:gap-8 lg:space-y-0">
          <div class="space-y-5 sm:space-y-4">
            <h2
              id="team"
              class="text-3xl font-extrabold tracking-tight sm:text-4xl"
            >
              Team
            </h2>
          </div>
          <div class="lg:col-span-2">
            <ul
              class="space-y-12 sm:divide-y sm:divide-gray-200 sm:space-y-0 sm:-mt-8 lg:gap-x-8 lg:space-y-0"
            >
              <li class="sm:py-8">
                <div
                  class="space-y-4 sm:grid sm:grid-cols-3 sm:items-start sm:gap-6 sm:space-y-0"
                >
                  <div
                    class="aspect-w-3 aspect-h-2 sm:aspect-w-3 sm:aspect-h-4"
                  >
                    <img
                      class="object-cover shadow-lg"
                      src="~/assets/tianzhou.webp"
                      alt=""
                    />
                  </div>
                  <div class="sm:col-span-2">
                    <div class="space-y-4">
                      <div class="text-lg leading-6 font-medium space-y-1">
                        <div class="flex space-x-2 items-center">
                          <h3>Tianzhou Chen</h3>
                          <ul class="flex space-x-2">
                            <li>
                              <a
                                href="https://twitter.com/tianzhouchen"
                                class="text-gray-400 hover:text-gray-500"
                              >
                                <span class="sr-only">Twitter</span>
                                <svg
                                  class="w-5 h-5"
                                  fill="currentColor"
                                  viewBox="0 0 20 20"
                                  aria-hidden="true"
                                >
                                  <path
                                    d="M6.29 18.251c7.547 0 11.675-6.253 11.675-11.675 0-.178 0-.355-.012-.53A8.348 8.348 0 0020 3.92a8.19 8.19 0 01-2.357.646 4.118 4.118 0 001.804-2.27 8.224 8.224 0 01-2.605.996 4.107 4.107 0 00-6.993 3.743 11.65 11.65 0 01-8.457-4.287 4.106 4.106 0 001.27 5.477A4.073 4.073 0 01.8 7.713v.052a4.105 4.105 0 003.292 4.022 4.095 4.095 0 01-1.853.07 4.108 4.108 0 003.834 2.85A8.233 8.233 0 010 16.407a11.616 11.616 0 006.29 1.84"
                                  />
                                </svg>
                              </a>
                            </li>
                            <li>
                              <a
                                href="https://www.linkedin.com/in/tianzhouchen/"
                                target="_blank"
                                class="text-gray-400 hover:text-gray-500"
                              >
                                <span class="sr-only">LinkedIn</span>
                                <svg
                                  class="w-5 h-5"
                                  fill="currentColor"
                                  viewBox="0 0 20 20"
                                  aria-hidden="true"
                                >
                                  <path
                                    fill-rule="evenodd"
                                    d="M16.338 16.338H13.67V12.16c0-.995-.017-2.277-1.387-2.277-1.39 0-1.601 1.086-1.601 2.207v4.248H8.014v-8.59h2.559v1.174h.037c.356-.675 1.227-1.387 2.526-1.387 2.703 0 3.203 1.778 3.203 4.092v4.711zM5.005 6.575a1.548 1.548 0 11-.003-3.096 1.548 1.548 0 01.003 3.096zm-1.337 9.763H6.34v-8.59H3.667v8.59zM17.668 1H2.328C1.595 1 1 1.581 1 2.298v15.403C1 18.418 1.595 19 2.328 19h15.34c.734 0 1.332-.582 1.332-1.299V2.298C19 1.581 18.402 1 17.668 1z"
                                    clip-rule="evenodd"
                                  />
                                </svg>
                              </a>
                            </li>
                          </ul>
                        </div>
                        <p class="text-indigo-600">Creator</p>
                      </div>
                      <div class="text-lg text-gray-500">
                        <p>
                          Tianzhou was the product head at Ant Group, led its
                          Database Platform, Developer Tools, and Productivity &
                          Collaboration group. He also served as the chair of
                          the company's coding guideline committee.
                        </p>
                        <p class="mt-2">
                          Before Ant Group, he was the technical lead of Google
                          Cloud SQL. He led the team to build its initial
                          PostgreSQL offering and was the main architect of the
                          current Cloud SQL service infrastructure. He was also
                          the primary maintainer of Google's PostgreSQL and
                          MySQL branch.
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
              </li>
              <!-- More people... -->
            </ul>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script lang="ts">
import { defineComponent, useMeta } from "@nuxtjs/composition-api";
import Plausible from "plausible-tracker";

const { trackEvent } = Plausible();

export default defineComponent({
  head: {},
  setup() {
    const track = (name: string) => {
      trackEvent(name);
    };

    useMeta({
      title: "About Us",
      meta: [
        {
          hid: "about",
          name: "about",
          content: "Built by the engineers behind Google Cloud SQL",
        },
      ],
    });

    return { track };
  },
});
</script>
