              ...
              >
                ...
                >
                  <div class="flex items-center space-x-4">
                    ...
                    >
                    <div
                      v-for="(tag, tagIndex) in error.tagList"
                      :key="tagIndex"
                    >
                      <span
                        v-if="tag == 'General'"
...
import { ErrorCategory, ErrorItem, ErrorTag } from "./errorTypes";
...
export default defineComponent({
  ...
  setup() {
    ...
    const tagItemCount = (tag: ErrorTag): number => {
      let count = 0;
      for (const errorCategory of ERROR_LIST) {
        for (const error of errorCategory.list) {
          if (tag == "All" || error.tagList.includes(tag)) {
            count++;
          }
        }
      }
    ...
    const filteredCategoryList = computed(() => {
      ...
      if (filterTagList.length > 0) {
        const list: ErrorCategory[] = [];
        for (const errorCategory of ERROR_LIST) {
          const errorList: ErrorItem[] = [];
          for (const error of errorCategory.list) {
            for (const tag of error.tagList) {
